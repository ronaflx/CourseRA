...
abstract class TweetSet {
  ...
  def mostRetweetedAcc(cur: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def mostRetweetedAcc(cur: Tweet): Tweet = cur
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(cur: Tweet): Tweet = {
    left.mostRetweetedAcc(right.mostRetweetedAcc(if (elem.retweets > cur.retweets) elem else cur))
  }
  def descendingByRetweet: TweetList = {
    def build(s : TweetSet) : TweetList = {
      if(s.empty) {
        Nil
      } else {
        new Cons(s.mostRetweeted, build(s.remove(s.mostRetweeted)))
      }
    }
    build(this)
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(
      tw => google.exists((s : String) => tw.text.contains(s))
  )
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(
      tw => apple.exists((s : String) => tw.text.contains(s))
  )
  /**
