...
def calcCYK(w):
    ...
    for i in range(L):
        for j in range(P):
            if w[i] == production[j][1] and production[j][2] == -1:
                X[i][i][production[j][0]] = True
    ...
    for l in range(2, L + 1):
        for i in range(0, L - l + 1):
            ...
            for k in range(i, j):
                for p in range(P):
                    if production[p][1] != -1 and production[p][2] != -1 and X[i][k][production[p][1]] and X[k + 1][j][production[p][2]]:
                       X[i][j][production[p][0]] = True
